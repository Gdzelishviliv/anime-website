...
export class RabbitMQService implements OnModuleInit {
  ...
  private async connect(retries = 1): Promise<void> {
    try {
      ...
      this.logger.error('Failed to connect to RabbitMQ — events will be unavailable', (error as Error).message);
    }
  }
  ...
  async subscribe(queue: string, routingKey: string, handler: (msg: any) => Promise<void>): Promise<void> {
    if (!this.channel) return;
